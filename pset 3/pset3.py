...
def get_word_score(word, n):
    ...
    first_component_total = 0
    for letter in word:
        first_component_total += SCRABBLE_LETTER_VALUES.get(letter, 0)
    
    second_component_total = (7 * len(word) - 3 * (n - len(word)))
    if second_component_total > 1:
        second_component_total = second_component_total
    else:
        second_component_total = 1
        
    final_total = first_component_total * second_component_total
    return final_total
...
def update_hand(hand, word):
    ...
    for letter in word:
        if letter not in hand or updated_hand[letter] == 0:
            continue
        else:
...
def is_valid_word(word, hand, word_list):
    ...
    if word not in word_list:
        ...
        for possible_word in possible_words_list:
            counter = 0
            for letter in word:
                if letter == possible_word[counter] or (letter == '*' and possible_word[counter] in VOWELS):
                    counter += 1
                else:
                    break
            if counter == len(word):
                possible_words_list_final.append(possible_word)

        if len(possible_words_list_final) <= 0:
            return False
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while handlen > 0:
        ...
        if word == '!!':
            ...
            if is_valid_word(word, hand, word_list):
                ...
                sum_scores += score
                print('"' + str(word) + '"', 'earned', score, 'points.', end = ' ')
                print('Total:', sum_scores, 'points')
            else:
                if handlen <= 1:
    ...
    print('Total score for this hand:', sum_scores)
...
def substitute_hand(hand, letter):
    ...
    if new_letters <= 0:
        ...
        if letter in VOWELS:
            possible_letters = VOWELS - hand_copy.keys()
        else:
            possible_letters =  CONSONANTS - hand_copy.keys()
        possible_letters = list(possible_letters)
        new_letter = random.choice(possible_letters)
        del(hand_copy[letter])
        hand_copy[new_letter] = hand_copy.get(new_letter, 0) + new_letters
